...
# Replace your current sources list with this expanded version
    ...
    def _safe_fetch_rss(self, source):
        ...
        try:
            ...
            # Process entries - increase to 800 for more content
            processed_entries = []
            for entry in feed.entries[:800]:
                try:
...
class HistoricalDataCollector:
    ...
    def _process_archive(self, source):
        ...
        try:
            ...
            processed_articles = []
            for link in links[:600]:  # Limit to 20 articles per archive to avoid overloading
                try:
...
def add_novelty_detection_page(dashboard, data_manager):
  ...
  # Store the novelty page
    ...
    dashboard.main_dashboard = dashboard.app.layout
    
    # Create a SINGLE tab-based layout (remove the duplicate tab_layout definition)
    tab_layout = html.Div([
        dcc.Tabs(
            id="dashboard-tabs", 
            value="main-dashboard", 
    ...
    # Register callback to switch between tabs - THIS IS THE IMPORTANT PART
    @dashboard.app.callback(
    ...
    def render_dashboard_content(tab):
        if tab == "novelty-detection":
            return dashboard.novelty_page  # Use the stored novelty page
        else:
            # Return the components from the main dashboard, not the entire layout
            # This may need to be adjusted based on your main dashboard structure
            return dashboard.main_dashboard.children  # Try using children first
